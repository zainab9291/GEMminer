...
for i=1:numel(Modelss)
    
%      kk=myFormatFontH1(FileNam,Modelss{i});
...
%     %fprintf(fptr,'<td>Copi vesiles</td>');
    ...
    fprintf(fptr,'<td>%s</td>',ModelsID{i});
    
    NUrl=['https://www.ncbi.nlm.nih.gov/pubmed/?term=' ModelsID{i}];
   
    fprintf(fptr,'<td><a href="%s" target="_blank">%s</a></td>',NUrl,ModelsID{i});
